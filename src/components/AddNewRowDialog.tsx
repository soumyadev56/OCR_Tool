...
interface AddNewRowDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onAdd: (newRow: Omit<TableData, 'id'>) => void;
}
...
const emptyRow = {
  ...
  swiftCode: "",
  upiId: "",
  uploadDateTime: new Date().toUTCString(),
  invoiceDate: "",
  taxInvoiceNo: "",
  gstNo: ""
};

const AddNewRowDialog: React.FC<AddNewRowDialogProps> = ({
  open,
  onOpenChange,
  onAdd,
}) => {
  const [formData, setFormData] = useState<Omit<TableData, 'id'>>({...emptyRow});
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onAdd(formData);
    setFormData({...emptyRow}); // Reset form
  };
...
export default AddNewRowDialog;
